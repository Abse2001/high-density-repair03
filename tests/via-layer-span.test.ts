...
test("expands blind and buried via endpoints across the complete inclusive layer span", () => {
  const layers = ["top", "inner1", "inner2", "bottom"]
  expect(
    getViaLayers({ from_layer: "top", to_layer: "bottom" }, 4, true),
  ).toEqual(layers)
  expect(
    getViaLayers({ from_layer: "bottom", to_layer: "top" }, 4, true),
  ).toEqual(layers)
  expect(
